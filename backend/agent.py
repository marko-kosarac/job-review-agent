...
from tools import analyze, generate_cover_letter, research_company, extract_company_info
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Extracts the company name and job title from the job posting that was already scraped",
...
STATUS_MESSAGES = {
    ...
    "extract_company_name": "Finding the company name and job title...",
...
2. Call extract_company_name to get the company name and job title
...
def _get_cached_analysis(db, job_url: str, cv_text: str, language: str) -> Analysis | None:
...
def _analysis_to_result(analysis: Analysis) -> dict:
    return {
        "id": analysis.id,
        "extract_company_name": analysis.company_name,
        "analyze_match": {"analysis": analysis.match_analysis},
...
def _persist(db, job_url: str, state: dict, result: dict) -> str:
    analysis = Analysis(
        job_url=job_url,
        company_name=state.get("company_name"),
        cv_text=state.get("cv_text"),
        job_text=state.get("job_text"),
        match_analysis=result.get("analyze_match", {}).get("analysis"),
...
async def run_agent_stream(cv_text: str, job_url: str, language: str = "en"):
    ...
    async def call_tool(tool_name: str, tool_args: dict):
        if tool_name == "scrape_job":
            ...
            info = await extract_company_info(state["job_text"])
    ...
    try:
        cached = await asyncio.to_thread(_get_cached_analysis, db, job_url, cv_text, language)
        if cached:
        ...
        while True:
            ...
            if not message.tool_calls:
                ...
                if "extract_company_name" in result:
                    result["extract_company_name"] = state.get("company_name")
                if "analyze_match" in result:
                    analysis_id = await asyncio.to_thread(_persist, db, job_url, state, result)
